...
接收公司表单引擎审批节点回调,通过阿里云 OOS 模板执行域名切换的 FastAPI 服务。

## 这个服务在做什么

业务场景:一次"域名切换"流程要走 3 个审批节点(node_1 → node_2 → node_3),每个节点通过时由公司表单引擎回调本服务,本服务接到回调后:
...
校验失败时会自动调表单引擎的"节点回退"接口把审批节点退回"待审批"状态。
...
当 node_1 校验失败时,本服务会反向调用表单引擎的 `rollback_node` 接口,把对应审批节点的状态从"已通过"退回"待审批",防止"审批通过但切换没真执行"的不一致状态。
...
│   ├── form_platform_client.py 公司表单引擎反查/节点回退
...
| 公司表单引擎 | 反查申请单 / 节点回退 | 平台方提供 base URL + token(长期有效) |
| 飞书机器人 | 告警通知 | 可选,不填走 dry-run 日志 |
